...
class FlowDiagnoser:
    def __init__(self, artifact_path='.'):
        required_files = [
            'selected_features.joblib', 'scaler.joblib', 'gatekeeper_model.joblib', 
            'specialist_model.joblib', 'main_label_encoder.joblib', 'attack_label_encoder.joblib'
        ]
...
def diagnose_flow(flow_features: dict) -> str:
    ...
    try:
        ...
        gatekeeper_prediction_numeric = DIAGNOSER.gatekeeper_model.predict(scaled_features)[0]
        gatekeeper_label = DIAGNOSER.main_label_encoder.inverse_transform([gatekeeper_prediction_numeric])[0]

        print(f"  [Python Debug] Gatekeeper prediction: {gatekeeper_label}")
        
        if gatekeeper_label == 'Benign':
            return "Benign"
        
        print("  [Python Debug] Gatekeeper flagged as Attack. Passing to Specialist...")
        specialist_prediction_numeric = DIAGNOSER.specialist_model.predict(scaled_features)[0]
        final_diagnosis = DIAGNOSER.attack_label_encoder.inverse_transform([specialist_prediction_numeric])[0]
